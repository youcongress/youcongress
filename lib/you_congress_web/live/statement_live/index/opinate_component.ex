defmodule YouCongressWeb.StatementLive.Index.OpinateComponent do
  @moduledoc """
  ...
  def update(assigns, socket) do
    ...
    {:ok,
     ...
     |> assign(:statement, assigns.statement)}
  end
  ...
  def render(assigns) do
    ...
    <div class="pb-2">
      <.form
        for={@form}
        id={"v#{@statement.id}-opinion-form"}
        phx-target={@myself}
        phx-change="validate"
        phx-submit="save"
      >
  ...
  defp save_opinion(socket, :edit, %{"content" => content}) do
    %{
      assigns: %{current_user: current_user, opinion: opinion, statement: statement, vote: vote}
    } = socket
  ...
  defp save_opinion(socket, :new, %{"content" => content}) do
    %{
      assigns: %{current_user: current_user, statement: statement, vote: vote}
    } = socket
  ...
  defp vote_params(params) do
    %{current_user: current_user, statement: statement, opinion_id: opinion_id, vote: vote} =
      params
