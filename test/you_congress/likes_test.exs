defmodule YouCongress.LikesTest do
  ...
  test "like/2 returns {:ok, :already_liked} when the user has already liked the opinion" do
    opinion = opinion_fixture()
    user = user_fixture()

    assert {:ok, :liked} = Likes.like(opinion.id, user)
    assert {:ok, :already_liked} = Likes.like(opinion.id, user)
    assert Likes.count(opinion_id: opinion.id) == 1
  end
  ...
  test "get_liked_opinion_ids/2 includes vote opinions without an opinions_statements row" do
    statement = statement_fixture()
    opinion = opinion_fixture()
    user = user_fixture()
