defmodule YouCongressWeb.MCPServer.VotesVerify do
  @moduledoc """
  Verify that a vote's answer (for/against/abstain) is correct through the MCP server.
  ...
  schema do
    field :vote_id, :integer, required: true
  ...
  @impl true
  def execute(%{vote_id: vote_id, status: status, comment: comment, model: model}, frame) do
    ...
    with {:ok, normalized_status} <- normalize_status(status),
      ...
      {:error, :relevance_not_verified} ->
        {:reply,
         Response.error(
           Response.tool(),
           "Cannot verify this vote yet: the quote's relevance to this statement must be verified or ai_verified first."
         ), frame}
