defmodule YouCongressWeb.StatementLiveQuotedSearchTest do
  ...
  describe "Quoted Search" do
    test "matches exact phrase with quotes", %{conn: conn} do
      _opinion = opinion_fixture(content: "The quick brown fox jumps")
      _other = opinion_fixture(content: "The fox jumps quickly")
    ...
    test "matches any order without quotes", %{conn: conn} do
      _opinion = opinion_fixture(content: "The quick brown fox")
    ...
    test "does NOT match if terms are present but not adjacent when quoted", %{conn: conn} do
      _opinion = opinion_fixture(content: "The brown quick fox")
